<template>
    <div class="bg-gray-600 p-3 min-h-[25rem] max-h-[25rem] overflow-y-auto ">
        <div class="space-y-4 ">
            <div v-for="message in messages" :key="messages.id">
                ...
                <h1 v-if="message?.sender_id == user_id" class="bg-blue-600 p-2 rounded text-gray-200 font-light ">
                    {{ message.body }}</h1>
                <h1 v-else class="bg-gray-700 p-2 rounded text-gray-200 font-light ">{{ message.body }}</h1>
            </div>
    ...
    <form v-on:submit.prevent="store" class="flex justify-between w-full">
        <textarea v-model="body" placeholder="Send Message..."
                  class="w-full outline-none  p-2 resize-none text-black"></textarea>
...
const store = async () => {
    ...
    const {data} = await axios.post('/message/convo/' + convoId.value, {body: body.value})
