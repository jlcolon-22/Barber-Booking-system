<template>
  <div>
    <button @click="show" class="font-medium text-green-500 hover:underline">Edit</button>
    ...
    >
      <div class="relative w-full max-w-2xl max-h-full">
        <!-- Modal content -->
        <form @submit.prevent="update" class="relative rounded-lg shadow bg-gray-700">
          <!-- Modal header -->
          <div
            class="flex items-start justify-between p-4 border-b rounded-t border-gray-600"
          >
            <h3 class="text-xl font-semibold text-white">Update Post</h3>
            <button
              @click="modal = !modal"
              type="button"
...
const props = defineProps(["data", "employees"]);
...
const update = async () => {
  ...
  await axios.post("/owner/post/" + parseJson(props.data)?.id, formData);
  alert("updated successfully");
  window.location.reload();
};
const show = () => {
  const parseProps = parseJson(props.data);
  data.name = parseProps.name;
  data.employee_id = parseProps.employee_id;
  data.price = parseProps.price;
  data.category = parseProps.category;
