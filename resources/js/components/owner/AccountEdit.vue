    ...
    >
      <div class="relative w-full max-w-2xl max-h-full">
        ...
        <form @submit.prevent="update" class="relative rounded-lg shadow bg-gray-700">
          ...
          >
            <h3 class="text-xl font-semibold text-white">Update Employee</h3>
            <button
              @click="modal = !modal"
              type="button"
...
const update = async () => {
  ...
  await axios.post("/owner/account/" + parseJson(props.data)?.id, formData);
  alert("updated successfully");
  window.location.reload();
};
const show = () => {
  const parseProps = parseJson(props.data);
  data.email = parseProps.email;
  data.firstname = parseProps.firstname;
  data.lastname = parseProps.lastname;
