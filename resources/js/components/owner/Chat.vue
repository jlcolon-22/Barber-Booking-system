<template>
    ...
    <form v-on:submit.prevent="store" class="flex justify-between w-full">
        <textarea v-model="body" placeholder="Send Message..."
                  class="w-full p-2 outline-none resize-none text-black"></textarea>
...
const store = async () => {
    ...
    const {data} = await axios.post('/owner/message/convo/' + convoId.value, {body: body.value})
...
const fetch = async () => {
    const {data} = await axios.get('/owner/all/message/' + convoId.value);
    messages.value = data;
